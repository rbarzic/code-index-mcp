...
class ResponseFormatter:
    ...
    @staticmethod
    def _get_duplicate_names_from_index(
        index_cache: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, set]:
        """
    ...
    def file_list_response(files: List[str], status_message: str) -> Dict[str, Any]:
        ...
        """
        return {"files": files, "status": status_message}

    @staticmethod
    def search_results_response(
        results: List[Dict[str, Any]], pagination: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if functions:
            for func in functions:
                if isinstance(func, str):
                    ...
                    if "calls" in processed_func and isinstance(
                        processed_func["calls"], list
                    ):
                        processed_func["calls"] = (
                            ResponseFormatter._resolve_qualified_names_in_relationships(
                                file_path,
                                processed_func["calls"],
                                duplicate_names["functions"],
                                index_cache,
                            )
                        )

                    if "called_by" in processed_func and isinstance(
                        processed_func["called_by"], list
                    ):
                        processed_func["called_by"] = (
                            ResponseFormatter._resolve_qualified_names_in_relationships(
                                file_path,
                                processed_func["called_by"],
                                duplicate_names["functions"],
        ...
        if classes:
            for cls in classes:
                if isinstance(cls, str):
                    ...
                    # Resolve qualified names in relationship fields
                    if "instantiated_by" in processed_cls and isinstance(
                        processed_cls["instantiated_by"], list
                    ):
                        processed_cls["instantiated_by"] = (
                            ResponseFormatter._resolve_qualified_names_in_relationships(
                                file_path,
                                processed_cls["instantiated_by"],
                                duplicate_names["functions"],
